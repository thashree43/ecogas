...
router.get('/get_agent',authenticateToken(true,false), (req, res, next) => AdminControllerInstance.getallagent(req, res, next));
router.patch('/updateapproval/:id',(req,res,next)=>AdminControllerInstance.updateapproval(req,res,next))
router.get('/admingetorders',authenticateToken(true,false),(req,res,next)=>AdminControllerInstance.getallorder(req,res,next))
